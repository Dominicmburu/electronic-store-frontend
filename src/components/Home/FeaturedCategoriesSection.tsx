  ...
  useEffect(() => {
    const fetchFeaturedProducts = async () => {
      try {
        ...
        const featuredPrinters = data.featuredPrinters.slice(0, 10); // Limit to 10 products
      ...
      <div className="container">
        ...
        <div className="row g-4">
          {isLoading ? (
                ...
                <div className={`card ${styles.categoryCard} h-100 text-center`}>
                  ...
                  <div className="card-body">
                    <h5
                      ...
                      style={{ width: "80%" }}
                    />
                    <p className="card-text skeleton-text" style={{ width: "90%" }} />
            ...
            featuredProductsMemo.map((product) => (
              <CategoryCard key={product.id} product={product} />
            ))
          ) : (
            <div className="col-12 text-center">
              <p>{noDataMessage || "No featured products available at the moment."}</p>
            </div>
          )}
        </div>
      </div>
